...
describe("AllPayAuction", function () {
  it("Testing all pay auction", async function () {;

    const Auction = await hre.ethers.getContractFactory("AllPayAuction");
    ...
    await auction.connect(bidder2).bid(ethers.utils.solidityKeccak256([ "uint", "bool", "bytes32" ], [ 25, true, secret ]), {value: 100});
